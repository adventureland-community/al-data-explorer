...
import { ItemInfo, TradeItemInfo } from "typed-adventureland";
        ...
        <Card sx={{ marginBottom: 2 }}>
            <CardContent>
                <Typography component="div">
                    This page shows market data from adventureland merchants using the merchant
                    endpoint from <a href="https://aldata.earthiverse.ca">earthiverse's aldata</a>
                    <br />
                    You can search for items, separating searches by either space or comma. Press
                    REFRESH DATA to refresh the data from the merchant endpoint.
                </Typography>
            </CardContent>
        </Card>
    );
...
function TradeItemRow({
    ...
    const buyItems = Object.entries(prices.buying.merchants)
        .flatMap(([merchantName, merchant]) => merchant.items.map((x) => ({ merchantName, ...x })))
    ...
    for (const merchant of merchants) {
        ...
        for (tradeSlot in merchant.slots) {
            ...
            if (item) {
                ...
                let buyingOrSelling: "buying" | "selling" = item.b ? "buying" : "selling";
